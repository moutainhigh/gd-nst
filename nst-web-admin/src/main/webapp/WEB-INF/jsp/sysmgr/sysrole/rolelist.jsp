...
<script type="text/javascript" src="${CONTEXT }js/datagrid-detailview.js"></script>
	...
	<script>
		...
		function rowformater(value, row, index) {
			var html = "";
			html = html+ "<gd:btn btncode='XTJSGL002'><a class='operate' href='javascript:;' onclick=updateAction('"+ row.roleID+ "');>修改</a></gd:btn>"
					+ "<gd:btn btncode='XTJSGL005'><a class='operate' href='javascript:;' onclick=assignRight('"+ row.roleID+ "');>权限管理</a></gd:btn>"
					+ "<gd:btn btncode='XTJSGL006'><a class='operate' href='javascript:;' onclick=showRole('"+ row.roleID + "');>查看用户</a></gd:btn>";
			return html;
		}
	...
	<body>
		<table id="dg" title="">
			<thead>
				<tr>
					...
					<th data-options="field:'roleName',width:250,align:'left',sortable:true">角色名称</th>
					<th data-options="field:'attribute',width:250,align:'center',sortable:true,formatter:attrformater">角色类型</th>
					<th data-options="field:'remark',width:250,align:'left',formatter:formaterLongText">备注</th>
					<th data-options="field:'123',width:250,align:'center',formatter: rowformater">操作</th>
				</tr>
			</thead>
		</table>
		<div id="tb" style="padding: 5px; height: auto">
			<form id="searchform" method="post">
				<div>
					角色名称: 
					<input class="easyui-searchbox" type="text" id="roleName" name="roleName" style="width: 150px"
							data-options="prompt:'请输入角色名称', searcher:function(value,name){query2(value,name)}">
					<gd:btn btncode="XTJSGL004"><a class="easyui-linkbutton" iconCls="icon-search" onclick="query()">查询</a></gd:btn>
					<a class="easyui-linkbutton" iconCls="icon-reload" onclick="cleardata()">重置</a>
				</div>
			</form>
			<div style="margin-bottom: 5px">
				<gd:btn btncode="XTJSGL001"><a class="easyui-linkbutton" iconCls="icon-add" onclick="addRoleAction()">新增</a></gd:btn>
				<gd:btn btncode="XTJSGL003"><a class="easyui-linkbutton" iconCls="icon-remove" onclick="delObj()">删除</a></gd:btn>
		...
		<div id="editDialog" class="easyui-dialog" style="width: 400px; height: 220px;" closed="true" modal="true" buttons="#dlg-buttonsEdit">
			<div id="dlg-buttonsEdit" style="text-align: center">
				<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-save'" id="btn-save" onclick="save()">保存</a> 
				<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#editDialog').dialog('close')">关闭</a>
			</div>
		</div>
		<!-- 分配权限 -->
		<div id="assignRightDialog" class="easyui-dialog" style="width: 800px; height: 400px;" closed="true" modal="true" buttons="#dlg-buttonsAssignRight">
