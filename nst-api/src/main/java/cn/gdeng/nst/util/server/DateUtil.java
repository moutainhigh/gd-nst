	...
	public static boolean  isDateIntervalOverFlow(String startDate, String endDate, int interval) {
		...
		if(startDate.isEmpty()){
			between = Double.MAX_VALUE;
			 ...
			 between =(double) ((DateUtil.formateDate(startDate, DateUtil.DATE_FORMAT_DATEONLY).getTime() - new Date().getTime())/DateUtil.DAY_MILLI);
		}else {
			 between =(double) ((DateUtil.formateDate(endDate, DateUtil.DATE_FORMAT_DATEONLY).getTime() -DateUtil.formateDate(startDate, DateUtil.DATE_FORMAT_DATEONLY).getTime())/DateUtil.DAY_MILLI);
		}
